...
class Game:
    ...
    def initialize_game_pieces(self): # Generated
        ...
        for column, piece_type in enumerate(back_rank_order):
            ...
            self.board[0][column] = piece_class(id_running, column + 1, 1, True)
            id_running += 1

        # White pawns (row 1)
        for column in range(8):
            self.board[1][column] = Pawn(id_running, column + 1, 2, True)
            id_running += 1

        # Black pawns (row 6)
        for column in range(8):
            self.board[6][column] = Pawn(id_running, column + 1, 7, False)
            id_running += 1

        # Black back rank (row 7) - same order
        for column, piece_type in enumerate(back_rank_order):
            piece_class = globals()[piece_type]
            self.board[7][column] = piece_class(id_running, column + 1, 8, False)
            id_running += 1
